<?php
	...
	class Mseguimiento extends CI_Model{
		...
		public function mracs($area, $estado) {
			$this->tecsur = $this->load->database('tecsur', TRUE);
			...
			if ($area != 0) {
				$this->tecsur->where('r.c_area_registra', $area);
				$this->tecsur->or_where('r.c_area_atiende', $area);
			}
			if ($estado == 1) {
			...
			if(!$query->result()){
				...
				foreach ($query->result() as $row) {
					...
					$data['data'][] = array($f_reporte, $x_tipo_estandar, $x_categoria, $x_area_registra, $x_area_atiende, $n_sst, $x_ubicacion, $x_descripcion, $x_recomendacion, $i_estado, $reporte);
				}
				$query->free_result();
			}
			$this->tecsur->close();
